...
//! (`NzbDrone.Core/Download/Clients/`), not radarr or the (V2-targeted, stale)
//! `terraform-provider-whisparr`. `NzbVortex` is eros-specific: capital-V
//! naming (`NzbVortex`/`NzbVortexSettings`, vs. radarr's `Nzbvortex`/
//! `NzbvortexSettings`) and `tvCategory` in place of `movieCategory`.
//! `TorrentDownloadStation`/`UsenetDownloadStation` share `DownloadStationSettings`,
//! which also keeps Sonarr's `tvCategory`/`tvDirectory` naming.
