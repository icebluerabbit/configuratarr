...
/// Read-only metadata about a folder that Whisparr knows about but has no movie mapped to.
#[nested]
pub struct UnmappedFolder {
    /// Folder name as reported by Whisparr.
...
pub struct RootFolder {
    ...
    #[key]
    pub path: String,
    /// Whether Whisparr can currently access the folder, reported by the API.
    #[wire(read_only)]
    pub accessible: Option<bool>,
    /// Available disk space in bytes, reported by the API.
    #[wire(read_only)]
    pub free_space: Option<i64>,
    /// Subdirectories Whisparr found that have no movie mapped to them.
    #[wire(read_only)]
