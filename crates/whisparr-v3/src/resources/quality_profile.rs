...
/// Quality metadata returned by the Whisparr catalogue.
///
/// Unlike Radarr, Whisparr's `Quality` has no `modifier` field — the eros
/// spec's `Quality` schema carries only `id`/`name`/`source`/`resolution`.
#[nested]
pub struct Quality {
    pub id: Option<i32>,
    /// Quality tier name, e.g. `"WEBDL-1080p"`.
    pub name: Option<String>,
    /// Source medium string, e.g. `"web"`, `"bluray"`, `"dvd"`.
    pub source: Option<String>,
    /// Vertical pixel resolution for this quality tier, e.g. `1080`.
...
/// items have a non-empty `items` list and a null `quality`. Whisparr walks
/// the ladder top-to-bottom when selecting an upgrade candidate.
#[nested]
pub struct QualityProfileItem {
    pub id: Option<i32>,
    /// Group or quality tier label displayed in the UI, e.g. `"HD Web"`.
    pub name: Option<String>,
    ...
    /// When `true`, Whisparr will accept releases at this quality tier.
    pub allowed: bool,
}
...
pub struct QualityProfile {
    ...
    #[key]
    pub name: String,
    /// When `true`, Whisparr will seek a better-quality release after the initial download.
    pub upgrade_allowed: bool,
    /// Id of the cutoff quality; Whisparr will not seek upgrades past this point.
    pub cutoff: i32,
