...
//!
//! A release profile lets you whitelist or blacklist releases by requiring or
//! ignoring specific terms (e.g. `"PROPER"`, `"REPACK"`, codec tags) anywhere
//! in the release title.
//!
//! The `required` and `ignored` fields are typed as `nullable` with no
//! element-type constraint in the OpenAPI spec. In practice Whisparr uses
//! them as arrays of strings; `Option<Json>` covers both the populated and
...
pub struct ReleaseProfile {
    ...
    #[default(0)]
    pub indexer_id: i32,
    /// Movie tag ids this profile applies to; resolved from
    /// `${ref.tag.<label>}` at apply. An empty list means the profile
    /// is applied globally to all movies.
    #[reference(tag)]
    pub tags: Vec<i32>,
}
