//! Provider-resource: notification (connection). Composes the shared `Provider`
//! envelope with the typed `NotificationProvider` variant, plus the per-event
//! toggles. Both composed pieces use `#[flatten]`, so their keys sit top-level
//! on the wire.
//!
//! `NotificationResource` in the eros spec is byte-identical to radarr's — it
//! keeps every `onMovie*` / `supportsOnMovie*` key even though this fork tracks
//! scenes. Confirmed via `get_resource crates/whisparr-v3/spec/whisparr-v3.json
...
pub struct Notification {
    ...
    pub on_grab: bool,
    /// Fire notification when a scene file is imported after download.
    pub on_download: bool,
    /// Fire notification when a file is upgraded to a higher-quality version.
    pub on_upgrade: bool,
    /// Fire notification when a scene file is renamed.
    pub on_rename: bool,
    /// Fire notification when a scene is added to the Whisparr library.
    pub on_movie_added: bool,
    /// Fire notification when a scene is deleted from the library.
    pub on_movie_delete: bool,
    /// Fire notification when a scene file is deleted.
    ...
    /// Fire notification when a Whisparr application update is available.
