...
//! `#[default(expr)]`. snake_case field names map to camelCase on the wire
//! automatically.
...
pub struct MediaManagement {
    ...
    /// Minimum free disk space in MB required on the destination before Whisparr will import.
    #[default(100)]
    ...
    /// "Import File Limit" — caps how many files a *manual* import considers
    /// within one folder (`ManualImportService.cs` truncates the candidate list
    /// to this many). Nothing to do with folder creation.
    #[default(100)]
    pub whisparr_folder_limit: i32,
}
