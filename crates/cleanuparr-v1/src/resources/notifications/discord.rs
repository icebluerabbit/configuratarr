...
pub struct DiscordProvider {
    ...
    /// `https://discordapp.com/api/webhooks/`. Masked on read.
    pub webhook_url: Option<SecretValue>,
    /// Override the webhook's posted username.
    pub username: Option<String>,
    /// Override the webhook's posted avatar image URL.
    pub avatar_url: Option<String>,
}

impl CustomSync for DiscordProvider {
    fn reconcile<'a>(
        client: &'a HttpClient,
        desired: &'a [Value],
        ...
        Box::pin(super::reconcile_provider::<Self>(
            client, desired, prune, execute, "discord", "Discord",
        ))
    }
}
