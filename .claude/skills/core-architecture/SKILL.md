...
resolver.rs      StaticEnv / RefSource traits + SystemEnv; RefId (Int(i64)|Str|Pending(IdShape))
                 — the bounded ref id; Pending = a not-yet-created id (preview / id-less create
                 response), substituting RefId::PENDING (-1) or PENDING_STR ("-1") per its IdShape
...
- **String/GUID ids** — the ref id is [`resolver::RefId`] (`Int(i64)|Str|Pending(IdShape)`); `RefStore`/`RefSource` are keyed by it, resolve substitutes `RefId::to_value()`. *arr stay `Int`. `Pending` is the not-yet-created id (a `plan` preview, or a create whose response carried no id). It carries an [`IdShape`] because the placeholder still has to type-check against the FK that consumes it: `Int` renders `RefId::PENDING` (`-1`), `Str` renders `PENDING_STR` (`"-1"`). Handing a `String` FK the integer aborts the whole plan with `expected string, got -1` — which is what a first `plan` against an empty GUID service used to do. The shape comes from `engine::id_shape`, which reads the resource's `#[id]` field and defaults to `Int`, **so a string-id resource must declare `#[id]`**; a hook that inserts its own `Pending` (jellyfin `user`, cleanuparr's instance/download-client reconciles) passes the shape explicitly. Use these, never a bare `-1`. **In conformance tests** the id type must match the FK's Rust type, and one double can't serve both: `core_testkit::check` resolves every `${ref}` through `DummyRefs` (`Int(1)`), so a GUID service uses `check_with_refs(.., &GuidRefs)` instead — an `Int` in a `String` FK fails decode with `expected string, got 1`.
- **Multi-target references** — `FieldDescriptor.reference` is `&'static [&'static str]`, so `#[reference(a, b)]` declares a *polymorphic* FK (cleanuparr's seeker instance id addresses a sonarr **or** radarr instance). This matters because `apply::apply_order` derives the dependency graph **only** from that metadata — `resolve::collect_refs` is not on the apply path, so a `${ref.…}` a field didn't declare creates no edge, and the resource can be applied before its target exists. Ties break alphabetically, so a missing edge breaks in one direction only and is easy to miss.
